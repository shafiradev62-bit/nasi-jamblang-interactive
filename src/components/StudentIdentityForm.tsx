  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 max-h-[90vh] overflow-y-auto">
        ...
        {/* ── OPENING SLIDE HEADER ── */}
        {!isLogin && (
          <div className="mb-5 text-center">
            <p className="text-[10px] font-bold uppercase tracking-widest text-primary mb-2">
              {isId ? "Literasi Sains Berbasis Etnoscience Cirebon" : "Cirebon Ethnoscience-Based Scientific Literacy Test"}
            </p>
            <h1 className="font-display text-xl font-bold text-foreground mb-1 leading-tight">
              {isId
                ? "Ayo Eksplorasi Sains melalui Budaya Cirebon!"
                : "Let's explore science through the culture of Cirebon!"}
            </h1>
            <p className="text-[12px] text-muted-foreground leading-relaxed">
              {isId
                ? "Selamat datang! Dalam kegiatan ini, kamu akan belajar sains dari budaya Cirebon yang unik. Kamu akan menemukan bagaimana makanan lokal, tradisi, kerajinan, dan alam dapat membantumu memahami sains dengan cara yang menyenangkan dan bermakna."
                : "Welcome! In this activity, you will learn about science from the unique culture of Cirebon. You will discover how local foods, traditions, crafts, and nature can help you understand science in a fun and meaningful way."}
            </p>
        ...
        <div className="mb-4">
          <h2 className="font-display text-lg text-foreground mb-1">
            {isLogin
              ...
              : (isId ? "Identitas Siswa" : "Student Identity")}
          </h2>
          <p className="text-[11px] text-muted-foreground">
            {isLogin
              ? (isId ? "Lanjutkan dengan data yang sudah terdaftar." : "Continue with your registered data.")
              : (isId ? "Isi data dirimu sebelum memulai." : "Fill in your details before starting.")}
          </p>
        </div>
        ...
        {isLogin && savedProfile ? (
          <div className="space-y-4">
            ...
            <button onClick={() => setIsLogin(false)} className="w-full py-2.5 bg-white border border-border/60 text-foreground/70 font-medium rounded-lg hover:bg-muted/30 transition-all text-[13px]">
              {isId ? "Daftar Akun Baru" : "Register New Account"}
            </button>
          </div>
        ) : (
